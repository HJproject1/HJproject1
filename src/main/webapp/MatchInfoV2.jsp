...
<head>
	...
	$.ajax(settings).done(function (awn) {
 	...
 	function prev(){
		if(<%=Month%>==1||<%=Month%>==0){
			location.href = "FcController?command=matchInfo&Year="+<%=Year-1%>+"&Month=12";
		}else {
			location.href = "FcController?command=matchInfo&Year="+<%=Year%>+"&Month="+<%=Month-1%>;
		}
	} 
 	function next(){
		if(<%=Month%>==12){
			location.href = "FcController?command=matchInfo&Year="+<%=Year+1%>+"&Month=1";
		}else {
			location.href = "FcController?command=matchInfo&Year="+<%=Year%>+"&Month="+<%=Month+1%>;
		}
	} 
	...
	<section class="bannertop">
		...
		<div class="mainSection">
			...
			<section class="MatchTab League">
				<ul class="tabUI">
					<li id="tabUI0" class="on"><a id="aTab0" onclick = "sub1()"> <img
					...
					<div class="mainMatchtop">
						...
						<div class="right">
							<form action = "FcController" onsubmit = "return doaction()">
								<input type = "hidden" name = "command" value="matchInfo">
								<select name = "Year">
