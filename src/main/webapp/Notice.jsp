...
<%=request.getAttribute("writeMSG") == null ? "" : request.getAttribute("writeMSG")%>
<%=request.getAttribute("deleteMSG") == null ? "" : request.getAttribute("deleteMSG")%>
<%
	int pageNum = 0;
	try{
	...
	<script type="text/javascript">
		...
		for (let i = 0; i < aTags.length; i++) {
		  aTags[i].addEventListener("click", function() {
		    let bno = this.parentNode.parentNode.cells[0].textContent;
		  });
		}
		let pageNum = <%=pageNum%>;
		let request = new XMLHttpRequest();
		function searchFunction() {
			request.open("Post", "./NoticeSearchServlet?searchValue=" + encodeURIComponent(document.getElementById("searchValue").value) + "&pageNum=<%=pageNum%>", true);
				...
				for(let i=0; i<result.length; i++){
					...
					for(let j=0; j<result[i].length; j++){
						...
						if (j === 1) {
					          ...
					          // 각 tr [i]번째  j[0]번 == bno 값을 숫자로 변환
					          let bnoValue = parseInt(result[i][0].value);
					          aTag.href = "FcController?command=noticeDetail&bno="+bnoValue+"&page="+pageNum;
					          aTag.textContent = result[i][j].value;
	...
	<div id="main">
		<div class="submain">
			<section class="top">
				<div class="f1 route">
					<span>share</span>
					<img src="https://www.fcseoul.com/resources/front/images/sub/common/ico_gt.png"/>
					<a href="">
						<img src="https://www.fcseoul.com/resources/front/images/sub/common/ico_sns1.png"/>
					</a>
				</div>
				<h3 class="f1">NOTICE</h3>
				<div class="f2">
					<button type="button" class="urlCopy">
						URL 복사
					</button>
					<script>
		...
		<div class="topbox">
			<select type="searchtype">
		 	   <option value="all" selected="selected">전체</option>
		 	   <option value="title">제목</option>
				<option value="content">내용</option>
			</select>
			<input type="text" id="searchValue" onkeyup="searchFunction()" placeholder="검색어를 입력해주세요.">
			<input onclick="searchFunction();" type="submit" value="검색">
		</div>
		<div class="tbwrap">
			<table>
				<thead>
		...
		<div style="clear:both;"></div>
			<div class="page">
				<a href="FcController?command=notice&page=<%=pageNum - 1%>"></a>
				<%
				for(int i=1; i<=maxCount; i++){
					if(pageNum == i){
				%>
					<a class="on" href="FcController?command=notice&page=<%=i %>"><%=i %></a>
				<%		
					} else {
				%>
					<a href="FcController?command=notice&page=<%=i%>"><%=i %></a>
				<%
					}
				}
				if(pageNum == maxCount){
				%>
				<a href="FcController?command=notice&page=<%=maxCount%>"></a>
				<%
				} else {
				%>
				<a href="FcController?command=notice&page=<%=pageNum + 1%>"></a>
				...
				<script>
					$(function() {
					  ...
					  $("#write").click(function(){
						 location.href="FcController?command=noticeWritePage"; 
					  });
					});
				</script>
			</div>
