...
<%@page import="matchInfoAPI.MatchInfoDto2"%>
...
<%
	MatchInfoDTO_1 matchList = (MatchInfoDTO_1)request.getAttribute("selectMatch");
...
<head>
	...
	<link rel="stylesheet" href="css/MatchResultV2.css">
...
<body>
	...
	<section class="bannertop">
		...
		<div class="mainSection">
			...
			<%
			boolean is_home = (matchList.getIs_home()).equals("TRUE");
			boolean is_home_win = ((String)matchList.getHome_winner()).equals("승");
			boolean is_home_lose = ((String)matchList.getHome_winner()).equals("패");
			boolean is_away_win = ((String)matchList.getAway_winner()).equals("패");
			boolean is_away_lose = ((String)matchList.getAway_winner()).equals("패");
			%>
			<section class = "mainP">
				<div class = "bgWrap">
					<div class = "topContent">
		              <span><%=matchList.getCompetition() + " " +matchList.getRound() %></span>
		              <span><%=matchList.getM_date() %></span>
		              <span>
		              	<i class="<%=is_home?"home":"away"%>">H</i>
		              	<p><%=matchList.getStadium() %></p>
		              </span>
					</div>
					<div class="mainContent">
						<div class = "line1">
							<div class="left">
									<span id="resultSeoul" class=<%=is_home_win?"win":is_home_lose?"lost":"draw"%>></span> <span
										class="teamName"><%=matchList.getHome_name() %></span> <span class="teamLogo"><img
										src="<%=matchList.getHome_logo()%>"
										alt=""></span> 
								</div>
	
								<div class="center">
									<span class="score"><i><%=matchList.getHome_fulltime()%></i></span>:<span class="score"><i><%=matchList.getAway_fulltime()%></i></span>
								</div>
	
								<div class="right">
									<span class="teamLogo"><img
										src="<%=matchList.getAway_logo()%>"
										alt=""></span> <span class="teamName"><i><%=matchList.getAway_name()%></i></span> <span
										id="resultOpp" class="<%=is_away_win?"win":is_away_lose?"lost":"draw"%>"></span>
								</div>
							</div>
						<div class="line2">
							<div class="left">
								전반&nbsp;<i><%=matchList.getHome_harf()%> </i>&nbsp;:&nbsp;<i><%=matchList.getAway_harf()%></i>
							</div>

							<div class="center"></div>

							<div class="right">
								후반&nbsp;<i><%=matchList.getHome_s_harf()%></i>&nbsp;:&nbsp;<i><%=matchList.getAway_s_harf()%></i>
							</div>
						</div>
					</div>
				</div>
