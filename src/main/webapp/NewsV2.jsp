...
<head>
	...
	$("document").ready(function() {  
		...
		function select(it){
		  const URLSearch = new URLSearchParams(location.search);
		  URLSearch.set('seq', it.parentElement.children[0].textContent);
		  URLSearch.set('command',"newsDetail");
		  const newParam = URLSearch.toString();
		  window.open(location.pathname + '?' + newParam, '_self');
		}
		function selectV(it){
	...
	<section class="bannertop">
	    ...
	    <section class = "mainpage">
			...
			<div class = "main_content">
				...
				<div class = "tapbutton">
					<div class = "listType">
						<a class = "list1" href = "FcController?command=news2">썸네일형</a>
						<a class = "list2 active" href = "FcController?command=news">리스트형</a>
					</div>
				</div>
				<div class = "tableWrap">
					<table>
				...
				for(NewsDTO nDTO : newsList) {
						...
						<tbody>
							<tr>
								<td class = "bno"><%= nDTO.getBno() %></td>
								<td class = "title" onclick = "select(this)">
									<a>
                    					<span class="news">News</span>
                    					<span><%=nDTO.getTitle() %></span>
                  					</a>
                  				</td>
								<td><%=nDTO.getWritedate() %></td>
							</tr>
						</tbody>
						<% }
							%>
