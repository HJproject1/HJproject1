...
<%
	QaDto qdto = (QaDto)request.getAttribute("qdto");
		...
		$(function(){
			$(document).ready(function() {
				$(".cancle").click(function() {
					location.href = "FcController?command=QADetail&bno=<%=qdto.getBno()%>&open=<%=qdto.getOpen()%>";
				})
			})
		...
		<div class="sns1">
			...
			<div class="writewrap">
				<form action="FcController" id="firm">
					...
					<input type="hidden" name="writer" value="<%=qdto.getWirter() %>">
					...
					<table>
						...
						<tr>
							...
							<td>
								<select name="type">
									<option value="FC서울 경기" <%=qdto.getCategory().equals("FC서울 경기")?"selected":"" %>>FC서울 경기</option>
									<option value="FC서울 선수단" <%=qdto.getCategory().equals("FC서울 선수단")?"selected":"" %>>FC서울 선수단</option>
									<option value="FC서울 유스팀" <%=qdto.getCategory().equals("FC서울 유스팀")?"selected":"" %>>FC서울 유스팀</option>
									<option value="뉴스 및 기사" <%=qdto.getCategory().equals("뉴스 및 기사")?"selected":"" %>>뉴스 및 기사</option>
									<option value="단일티켓" <%=qdto.getCategory().equals("단일티켓")?"selected":"" %>>단일티켓</option>
									<option value="단체입장권" <%=qdto.getCategory().equals("단체입장권")?"selected":"" %>>단체입장권</option>
									<option value="시즌티켓(어린이회원, 티켓북)" <%=qdto.getCategory().equals("시즌티켓(어린이회원, 티켓북)")?"selected":"" %>>시즌티켓(어린이회원, 티켓북)</option>
									<option value="스카이박스" <%=qdto.getCategory().equals("스카이박스")?"selected":"" %>>스카이박스</option>
									<option value="법인시즌티켓" <%=qdto.getCategory().equals("법인시즌티켓")?"selected":"" %>>법인시즌티켓</option>
									<option value="할인 예매권/커뮤니티 티켓" <%=qdto.getCategory().equals("할인 예매권/커뮤니티 티켓")?"selected":"" %>>할인 예매권/커뮤니티 티켓</option>
									<option value="멤버십(GS&PIONT)" <%=qdto.getCategory().equals("멤버십(GS&PIONT)")? "selected":"" %>>멤버십(GS&PIONT)</option>
									<option value="이벤트 통합 문의" <%=qdto.getCategory().equals("이벤트 통합 문의")?"selected":"" %>>이벤트 통합 문의</option>
									<option value="승리버스" <%=qdto.getCategory().equals("승리버스")?"selected":"" %>>승리버스</option>
									<option value="쇼핑몰(온라인 쇼핑몰, 팬파크)" <%=qdto.getCategory().equals("쇼핑몰(온라인 쇼핑몰, 팬파크)")?"selected":"" %>>쇼핑몰(온라인 쇼핑몰, 팬파크)</option>
									<option value="스폰서" <%=qdto.getCategory().equals("스폰서")?"selected":"" %>>스폰서</option>
									<option value="구단 관련 문의" <%=qdto.getCategory().equals("구단 관련 문의")?"selected":"" %>>구단 관련 문의</option>
									<option value="SNS" <%=qdto.getCategory().equals("SNS")?"selected":"" %>>SNS</option>
								</select>
							</td>
						</tr>
						<tr>
						...
						<tr>
							<th>제목</th>
							<td>
								<input type="text" name="title" id="qatitle" required value="<%=qdto.getTitle()%>">
							</td>
						</tr>
						<tr>
							<th>내용</th>
							<td>
								<textarea rows="20" cols="30" style="width:100%;" id="textwrite" name="textwrite"><%=qdto.getContent()%></textarea>
							</td>
						</tr>
					</table>
		...
		$(function() {
			$(document).ready(function() {
				$(".reg").click(function() {
					...
					location.href = "FcController?command=QAUpdate&bno=<%=qdto.getBno()%>";
					$("#firm").submit();
				})
			})
		})
