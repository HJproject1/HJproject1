...
<head>
	...
	$.ajax(settings).done(function (awn) {
 	...
 	function prev(){
		if(<%=Month%>==1||<%=Month%>==0){
			location.href = "FcController?command=matchInfo2&Year="+<%=Year-1%>+"&Month=12";
		}else {
			location.href = "FcController?command=matchInfo2&Year="+<%=Year%>+"&Month="+<%=Month-1%>;
		}
	} 
 	function next(){
		if(<%=Month%>==12){
			location.href = "FcController?command=matchInfo2&Year="+<%=Year+1%>+"&Month=1";
		}else {
			location.href = "FcController?command=matchInfo2&Year="+<%=Year%>+"&Month="+<%=Month+1%>;
		}
	} 
					...
					<div class="mainMatchtop">
						...
						<div class="right">
							<form action = "FcController" onsubmit = "return doaction()">
								<input type = "hidden" name = "command" value="matchInfo2">
								<select name = "Year">
