<%@page import="recordRankAPI.recordRankDTO"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	ArrayList<recordRankDTO> rankingList = (ArrayList<recordRankDTO>)request.getAttribute("rankingList");
...
<head>
	...
	$("document").ready(function() {  
		$("#Year").val("<%=rankingList.get(0).getYear()%>").prop("selected", true);
			...
			<section class = "mainRecord">
				<form id = "searchForm" name = "form" method = "GET" action = "FcController">
					<div class = "recordCategory">
						...
						<div class = "right">
							<input type = "hidden" name = "command" value = "rankingList">
				...
				<div class = "recordTable">
					<table>
						...
						<tbody>
						<%
							for(recordRankDTO rRDTO : rankingList){
						%>
			                <tr class = <%=rRDTO.getTeam().equals("FC Seoul")?"on":""%>>
			                  <td><%=rRDTO.getRank()%>위</td>
							  <td><%=rRDTO.getTeam()%></td>
			                  <td><%=rRDTO.getGames()%></td>
			                  <td><%=rRDTO.getPoint() %></td>
			                  <td><%=rRDTO.getWin() %></td>
							  <td><%=rRDTO.getDraw()%></td>
			                  <td><%=rRDTO.getLose()%></td>
			                  <td><%=rRDTO.getGoals() %></td>
							  <td><%=rRDTO.getConced()%></td>
			                  <td><%=rRDTO.getGoals_diff()%></td>
			                </tr>
			                <%
			            }
						%>
