<%@page import="recordRankAPI.rankingScoreDTO"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	ArrayList<rankingScoreDTO> scoreList = (ArrayList<rankingScoreDTO>)request.getAttribute("ScoreList");
...
<head>
	...
	$("document").ready(function() {  
		$("#Year").val("<%=scoreList.get(0).getYear()%>").prop("selected", true);
			...
			<section class = "mainRecord">
				<form id = "searchForm" name = "form" method = "GET" action = "FcController">
					<div class = "recordCategory">
						...
						<div class = "right">
							<input type = "hidden" name = "command" value = "rankingScore">
				...
				<div class = "recordTable">
					<table>
		              <thead>
						...
						<tbody>
						<%
						for (rankingScoreDTO rSDTO : scoreList){
						%>
			                <tr>
			                  <td class="point"><%=rSDTO.getRank() %>위</td>
							  <td><%=rSDTO.getName() %></td>
			                  <td><%=rSDTO.getTeam() %></td>
			                  <td><%=rSDTO.getGoals()%></td>
			                  <td><%=rSDTO.getGames() %></td>
			                  <td><%=rSDTO.getNum_replace() %></td>
			                  <td><%=rSDTO.getgoals_game() %></td>
			                </tr>
			                <%
			            }
						%>
