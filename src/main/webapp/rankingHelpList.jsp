<%@page import="recordRankAPI.rankingHelpDTO"%>
<%@page import="java.util.ArrayList"%>
<%@page import="recordRankAPI.RankingHelpDAO"%>
<%@page import="recordRankAPI.RankingScoreDAO"%>
<%@page import="java.sql.ResultSet"%>
<%@page import="java.sql.PreparedStatement"%>
<%@page import="java.sql.Connection"%>
<%@page import="recordRankAPI.RecordRankDAO"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	ArrayList<rankingHelpDTO> helpList = (ArrayList<rankingHelpDTO>)request.getAttribute("helpList");
...
<head>
	...
	<link rel="stylesheet" href="css/RecordV2.css">
	...
	$("document").ready(function() {  
		...
		$("#Year").val("<%=helpList.get(0).getYear()%>").prop("selected", true);
...
<body>
	...
	<main>
		...
		<div class="mainSection">
			...
			<section class="tapWrap">
				<ul class="tabUI">
					<li id="li1">
						<a id="Btn1" href="FcController?command=rankingList">종합순위</a>
					</li>
					<li id="li2">
						<a id="Btn2" href="FcController?command=rankingScore">득점순위</a>
					</li>
					<li id="li3" class = "on">
						<a id="Btn3" href="FcController?command=rankingHelpList">도움순위</a>
					</li>
				</ul>
			</section>
			
			<section class = "mainRecord">
				<form id = "searchForm" name = "form" method = "GET" action = "FcController">
					<input type = "hidden" name = "command" value = "rankingHelpList">
					<div class = "recordCategory">
						...
						<div class = "right">
							<input type = "submit" value = "검색" onclick = "submitYear">
						</div>
					</div>
				</form>
				<div class = "recordTable">
					<table>
		              <thead>
						...
						<tbody>
						<%
							for(rankingHelpDTO rDTO : helpList){
						%>
			                <tr>
			                  <td class="point"><%=rDTO.getRank() %>위</td>
							  <td><%=rDTO.getName() %></td>
			                  <td><%=rDTO.getTeam() %></td>
			                  <td><%=rDTO.getAssist() %></td>
			                  <td><%=rDTO.getGames() %></td>
			                  <td><%=rDTO.getNum_replace() %></td>
			                  <td><%=rDTO.getAssist_game() %></td>
			                </tr>
			                <%
			            }
						%>
