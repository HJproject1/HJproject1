...
<%
	...
	int pageFull = (int) request.getAttribute("pageFull");
	int currPage = (int) request.getAttribute("currPage");
	ArrayList<NewsDTO> ThreeList = (ArrayList<NewsDTO>) request.getAttribute("ThreeList");
...
<head>
	...
	<link rel="stylesheet" href="css/NewsV2.css">
	...
	$("document").ready(function() {  
		...
		$(".pageNumWrap a").removeClass('on');
		$("#p<%=currPage%>").addClass('on');
		...
		$(function(){
			...
			function paging(it) {
			  ...
			  URLSearch.set('pageNum', it.getAttribute('data-value'));
			  const newParam = URLSearch.toString();
			  window.open(location.pathname + '?' + newParam, '_self');
			}
			function select(it){
			  const URLSearch = new URLSearchParams(location.search);
			  URLSearch.set('seq', it.firstElementChild.value);
		  	  URLSearch.set('command',"newsDetail2");
			  const newParam = URLSearch.toString();
			  window.open(location.pathname + '?' + newParam, '_self');
			}
			function selectV(it){
...
<body>
	...
	<section class="bannertop">
						...
						<ul>
							...
							<li>
								...
								<ul id="buttonUI2" class = button123 style="display: none;">
									<li><a href="FcController?command=news2">NEWS</a>
									</li>
									<li><a href="FcController?command=notice">NOTICE</a>
									</li>
									<li><a href="FcController?command=photo">PHOTO</a>
									</li>
								</ul>
							</li>
						</ul>
	    ...
	    <section class = "mainpage">
			<div class = "representive">
				<div class = "left" onclick = "selectV(this)" style = "cursor : pointer">
							<input type="hidden" name="bno" id = "bno" value="<%=ThreeList.get(0).getBno()%>">
					<div class = "leftimage">
						<a>
							<img onerror="this.src='/resources/front/images/common/news_noimages.jpg';" src="<%=ThreeList.get(0).getImage()%>">
						</a>
					</div>
					<div class = "lefttxt">
						<p class="tv">News</p>
						<p class="txt"><%=ThreeList.get(0).getTitle()%></p>
						<p>
								<span class="date fff"><%=ThreeList.get(0).getWritedate()%></span>
						</p>
					</div>
				</div>
				...
				<div class = "right">
					<ul>
					<%for(NewsDTO ThreeDto : ThreeList){ %>
						<li>
							<a onclick = "selectV(this)" style = "cursor : pointer">
								<input type="hidden" name="bno" id = "bno" value="<%=ThreeDto.getBno()%>">
								<div class = "rightimage">
									<img onerror="this.src='/resources/front/images/common/news_noimages.jpg';" 
									src="<%=ThreeDto.getImage()%>">
								</div>
								<div class = "righttxt">
									<p class="news">News</p>
									<p class="txt"><%=ThreeDto.getTitle() %></p>
									<p class ="lastp"><span class="date"><%=ThreeDto.getWritedate() %></span></p>
								</div>
							</a>
						</li>
						<%}
			...
			<div class = "main_content">
				<div class = "searchbox">
					<form action="FcController" method="GET">
				        ...
				        <input type="hidden" name = "command" value = "news2">
				...
				<div class = "tapbutton">
					<div class = "listType">
						<a class = "list1 active" href = "FcController?command=news2">썸네일형</a>
						<a class = "list2" href = "FcController?command=news">리스트형</a>
					</div>
				</div>
				<div class = "plWrap">
					<ul>
					<%
					for(NewsDTO nDTO : newsList) {
						%>
						<li>
						<a href="#" onclick = "select(this)">
			                <input type="hidden" name="bno" id = "bno" value="<%=nDTO.getBno()%>">
			                <div class="imageWrap">
			                  <img src="<%=nDTO.getImage()%>">
			                  <span class="news">News</span>
			                </div>
			
			                <div class="txtWrap">
			                  <p class="txt"><%=nDTO.getTitle() %></p>
			                  <p>
			                    <span class="date"><%=nDTO.getWritedate() %></span>
			                  </p>
			                </div>
			              </a>
						</li>
				...
				<div class="pageNumWrap">
				  <a class = "noHover" onclick = "paging(this)" data-value="<%=currPage-1%>">이전</a>
				  <%for(int i = 0; i< pageFull; i++){ %>
				  <a id = p<%=i+1%> onclick = "paging(this)" data-value="<%=i+1%>" class="nump"><%=i+1%></a>
				  <%} %>
				  <a class = "noHover" onclick = "paging(this)" data-value="<%=currPage==pageFull?currPage:currPage+1%>">다음</a>
				</div>
				<div class = "btnWrap">
					<a href = "FcController?command=newsWrite" class = "write"><span>쓰기</span></a>
				</div>
			</div>
		</section>
	</div>
