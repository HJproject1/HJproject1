...
<%@ page import="teamMember.teamMemberDAO"%>
...
<%
	...
	teamMemberDAO tmDAO = (teamMemberDAO)request.getAttribute("tmDAO");
%>
<!DOCTYPE html>
<html>
<head>
	...
	<link rel="stylesheet" href="css/teamMemberV2.css">
	...
	$("document").ready(function() {  
		...
		if(urlParams.has('FW')){
			...
			$("#FW").addClass("on");
			$(".teamPlayer ul li").removeClass("on");
			$(".FW").addClass("on");
			$(".teamPlayer h4").text("FW");
		}
		else if(urlParams.has('MF')){
			$(".teamTab li").removeClass("on");
			$("#MF").addClass("on");
			$(".teamPlayer ul li").removeClass("on");
			$(".MF").addClass("on");
			$(".teamPlayer h4").text("MF");
		}
		else if(urlParams.has('DF')){
			$(".teamTab li").removeClass("on");
			$("#DF").addClass("on");
			$(".teamPlayer ul li").removeClass("on");
			$(".DF").addClass("on");
			$(".teamPlayer h4").text("DF");
		}
		else if(urlParams.has('GK')){
			$(".teamTab li").removeClass("on");
			$("#GK").addClass("on");
			$(".teamPlayer ul li").removeClass("on");
			$(".GK").addClass("on");
			$(".teamPlayer h4").text("GK");
		}
		else {
			$(".teamTab li").removeClass("on");
			$("#FW").addClass("on");
	...
	<section class="bannertop">
		...
		<div class="mainSection">
			...
			<section class = "teamTab">
				<ul>
					<li id = "FW"><a href = "teamMember.jsp?FW">FW</a></li>
					<li id = "MF"><a href = "teamMember.jsp?MF">MF</a></li>
					<li id = "DF"><a href = "teamMember.jsp?DF">DF</a></li>
					<li id = "GK"><a href = "teamMember.jsp?GK">GK</a></li>
					<li><a class = "none">군 입대 및 임대</a></li>
					<li><a class = "none">코칭스태프</a></li>
					<li><a class = "none">메디컬스태프</a></li>
					<li><a class = "none">지원스태프</a></li>
				</ul>
			</section>
			<section class = "teamPlayer">
				<h4>FW</h4>
				<ul>
			<%
				Connection conn = tmDAO.getConnection();
				String sql = "SELECT * FROM player";
				PreparedStatement pstmt = conn.prepareStatement(sql); 
				ResultSet rs = pstmt.executeQuery();
				while(rs.next()){
			%>
					<li class = "<%=rs.getString("position")%>">
						<a href = "teamMemberDetail.jsp">
							<div class="imageWrap">
								<img src="Images/player/<%=rs.getString("back_num") %><%=rs.getString("position")%><%=rs.getString("name")%>.png" />
							</div>
							<div class="txtWrap">
								<p class="left"><%=rs.getString("back_num") %></p>
								<span class="right">
									<em><%=rs.getString("position") %></em>
									<%=rs.getString("name") %>
								</span>
							</div>
						</a>
