...
<%@ page import="java.util.ArrayList"%>
<%=request.getAttribute("updateMSG") == null ? "" : request.getAttribute("updateMSG")%>
<%
...
<body>
						...
						<ul>
							...
							<li>
								...
								<ul id="buttonUI2" class = button123 style="display: none;">
									...
									<li><a href="FcController?command=notice&category=all">NOTICE</a>
									</li>
	...
	<div id="main">
		...
		<div class="top2">
			...
			<div class="middle">
				<div>
					<p style="text-align: center; ">
						<img id="con" src="${content}"/>
					</p>
					<p style="text-align: center; margin-top: 26px;" href="https://www.g2b.go.kr/pt/menu/selectSubFrame.do?framesrc=/pt/menu/frameTgong.do?url=https://www.g2b.go.kr:8101/ep/tbid/tbidList.do?taskClCds=&bidNm=fc%BC%AD%BF%EF&searchDtType=1&fromBidDt=2022/11/29&toBidDt=2022/12/29&fromOpenBidDt=&toOpenBidDt=&radOrgan=1&instNm=&area=&regYn=Y&bidSearchType=1&searchType=1">
			...
			<div class="btnWrap">
				<a class="bg" href="FcController?command=notice&page=${pageNum}">목록</a>
				<a class="bg" id="upt" href="FcController?command=noticeUpdatePage&bno=${bno}">수정</a>
				<a class="br"id="del" href="FcController?command=noticeDelete&bno=${bno}">삭제</a>
				<script>
					$(function() {
					  // 세션 값 가져오기
				...
				<tbody>
					<% 
						for(NoticeDTO nDto : noticeList){
							...
							int w_date = nDto.getW_date();
					%>
					<tr>
						<td class="bno"><%=bno %></td>
						<td class="title">
							<a href="FcController?command=noticeDetail&bno=<%=bno%>">
								<span><%=title %></span>
							</a>
						</td>
						<td><%=w_date %></td>
		...
		<div style="clear:both;"></div>
			<div class="page">
				<a id="minus" href="FcController?command=notice&page=<%=pageNum - 1%>"></a>
				<%for(int i=1; i<=6; i++){
					if(pageNum == i){
				%>
				...
				%>
				<a id="plus" href="FcController?command=notice&pageNum=<%=pageNum + 1%>"></a>
			</div>
		</div>
	</div>
	<div style="clear:both;"></div>
