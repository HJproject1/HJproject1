...
<%
    	...
    	ArrayList<SocialBoardDto> sbMainList = (ArrayList<SocialBoardDto>)request.getAttribute("sbMainList");
...
<head>
	...
	<link rel="stylesheet" href="css/SocialBoard.css">
...
<%-- 					<input type="hidden" name="id" value="<%=id %>"/> --%>
					...
					<input type="hidden" name="command" value="socialBoard"/>
					<input type="hidden" name="searchBtn" value="검색"/>
			...
			<table>
				...
				%>
					<tr>
						<td><%=sbDto.getBno() %></td>
						<td> <a href="FcController?bno=<%=sbDto.getBno()%>&page=<%=pageNum%>&command=socialBoardDetail"><%=sbDto.getTitle() %></a></td>
						<td>
							<div class="writer">
			...
			<div class="sociallast">
				<div class="numpage">
					<a href="FcController?page=<%=pageNum-1%>&searchBtn=&search=&command=socialBoard"></a>
					<%
					...
					for(int i=1; i<=maxCount; i++) {
						if(pageNum == i) {
						%>
							<a class="pnum on" href="FcController?page=<%=i%>&searchBtn=&search=&command=socialBoard"><%=i %></a>
						<%
						} else {
						%>
							<a class="pnum" href="FcController?page=<%=i%>&searchBtn=&search=&command=socialBoard"><%=i %></a>
						<%	
						}
					}
					if (pageNum == maxCount) {
					%>
						<a href="FcController?page=<%=maxCount%>&searchBtn=&search=&command=socialBoard"></a>
					<%
					} else {
					%>
						<a href="FcController?page=<%=pageNum+1%>&searchBtn=&search=&command=socialBoard"></a>
				...
				<div class="writebtn">
					<a href="FcController?page=<%=pageNum%>&searchBtn=&search=&command=socialWrite">
						<span>쓰기</span>
					</a>
				</div>
			</div>
