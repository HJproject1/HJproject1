...
<head>
	...
	<link rel="stylesheet" href="css/QaDetail2.css">
    ...
    <script >
		...
		$(function(){
			...
			$(document).ready(function() {
				...
				$(".wbtn").click(function () {
					let bno = <%=qdto.getBno()%>;
				//	location.href = "QaAnswer.jsp?bno="+bno;
					location.href = "FcController?command=QAAnswerForm&bno="+bno;
				});
		...
		$(document).ready(function() {
			$(".del").click(function() {
				...
				if(confirm("삭제하시겠습니까?")==true){
					/* location.href = "QaDeleteAction.jsp?bno="+bno; */
					location.href = "FcController?command=QADelete&bno="+bno;
				}else{
					return false;
				}
			})
			$(".cor").click(function () {
				let bno = <%=qdto.getBno()%>
				location.href = "FcController?command=QAUpdateform&bno="+bno;
			})
		})
...
<body>
	...
	<section class="bannertop">
		...
		<div class="sns1">
			...
			<div class="board">
				<div class="board1">
					<span class=" <%=qdto.getAnswer().equals("답변완료") ? " fin" : " wait"%>"><%=qdto.getAnswer() %></span>
					<p class="title"><%=qdto.getTitle() %></p>	
					<ul class="writeinfo"> 
						<li class="date"><%=qdto.getW_date() %></li>
					</ul>
				</div>
				<div class="qacontent">
					<p><%=qdto.getContent()%></p>
				...
				<div class="contentbtn">
					...
					<button class="cor" style="display: <%= id.equals(qdto.getWirter()) ? "block":"none"%>">수정</button>	
					<button class="del" style="display: <%= id.equals(qdto.getWirter())||id.equals("operator") ? "block":"none"%>">삭제</button>	
					<button class="wbtn" style="display: <%= id.equals("operator") ? "block":"none"%>">쓰기</button>
				</div>
			</div>
