<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@page import="matchInfoAPI.MatchInfoDTO_1"%>
<%@page import="java.util.ArrayList"%>
<%@page import="java.util.Objects"%>
<%@page import="java.net.ConnectException"%>
<%@page import="matchInfoAPI.MatchInfoDAO2"%>
...
<%
	...
	ArrayList<MatchInfoDTO_1> matchList = (ArrayList<MatchInfoDTO_1>)request.getAttribute("showMatchInfo");
	int Month = (int) request.getAttribute("Month");
	int Year = (int) request.getAttribute("Year");
	String MonthValue = (String) request.getAttribute("MonthValue");
	String dtD = (String) request.getAttribute("dtD");
...
<head>
	...
	<link rel="stylesheet" href="css/MatchInfoV2.css">
    ...
    <script>
		// api 데이터 업데이트 유무 확인
		var Dated = `<%out.print(dtD);%>` ;
		console.log(Dated);
		const settings = {
		async: true,
		crossDomain: true,
		url: 'https://api-football-v1.p.rapidapi.com/v3/fixtures?date='+Dated+'&season=2023&team=2766',
		method: 'GET',
		headers: {
	...
	$.ajax(settings).done(function (awn) {
		console.log(awn.response);
		 $.ajax({
		     method: 'post',
		     url: 'MatchInfoServlet3',
		     traditional: true,
		     data: {
 	...
 	function prev(){
		if(<%=Month%>==1||<%=Month%>==0){
			location.href = "FcController?command=matchInfo3&Year="+<%=Year-1%>+"&Month=12";
		}else {
			location.href = "FcController?command=matchInfo3&Year="+<%=Year%>+"&Month="+<%=Month-1%>;
		}
	} 
 	function next(){
		if(<%=Month%>==12){
			location.href = "FcController?command=matchInfo3&Year="+<%=Year+1%>+"&Month=1";
		}else {
			location.href = "FcController?command=matchInfo3&Year="+<%=Year%>+"&Month="+<%=Month+1%>;
		}
	} 
 	...
 	function sub1(){
 		location.href = "FcController?command=matchInfo";
 	}
 	function sub2(){
 		location.href = "FcController?command=matchInfo2";
 	}
 	function sub3(){
 		location.href = "FcController?command=matchInfo3";
 	}
	...
	$(document).ready(function() {  
	    ...
	    $(".info4 a.homeM").text("티켓 예매");
	    $(".info4 a.awayM").text("FC 서울 원정 경기");
		$(".bottom .yesgame ul li .info2 p:last-child i.home").text("H");
		$(".bottom .yesgame ul li .info2 p:last-child i.away").text("A");
	});
</script>
</head>
<body>
	...
	<section class="bannertop">
						...
						<ul>
							...
							<li>
								...
								<ul id="buttonUI2" class = button123 style="display: none;">
									<li><a href="FcController?command=matchInfo">일정/결과</a>
									</li>
									<li><a href="FcController?command=rankingList">기록/순위</a>
									</li>
								</ul>
							</li>
						</ul>
		...
		<div class="mainSection">
			<section class="subtop">
				...
				<h3 class="h3">일정/결과</h3>
				<div class="right">
					<button type="button">URL 복사</button>
				</div>
			</section>
			<section class="MatchTab League">
				<ul class="tabUI">
					<li id="tabUI0"><a id="aTab0" onclick = "sub1()"> <img
							src="https://www.fcseoul.com/resources/front/images/sub/common/ico_league_all_on.jpg"
							alt="전체일정"> <span>전체일정</span>
					</a></li>
					<li id="tabUI1"><a id="aTab1" onclick = "sub2()"> <img
							src="https://fcseoulpub.s3.ap-northeast-2.amazonaws.com/multi01/Match/League/1673574666175.png"
							alt="K리그1"> <span>K리그1</span>
					</a></li>
					<li id="tabUI2" class= "on"><a id = "aTab2" onclick = "sub3()"><img
						src="https://fcseoulpub.s3.ap-northeast-2.amazonaws.com/multi01/Match/League/1680826377726.png"
						alt="FA컵"> <span>FA컵</span>
					</a></li>
				</ul>
			</section>
			<section class="mainMatchUp">
				<div class="mainMatchUpline"></div>
				<div class="mainMatchUpPage">
					<div class="mainMatchtop">
						<div class="left">
							<ul>
								<li class="homecolor">home</li>
								<li class="awaycolor">away</li>
							</ul>
						</div>
						<div class="center">
							<button class="prevButton" onclick = "prev()"></button>
								<span><%=Month==0?"전체":MonthValue+"월"%></span>
							<button class="nextButton" onclick = "next()"></button>
						</div>
						<div class="right">
							<form action = "FcController" onsubmit = "return doaction()">
								<input type = "hidden" name = "command" value="matchInfo3">
								<select name = "Year">
					...
					<div class="bottom">
						<ul>
							<li class="yesgame">
								<div class="yyyymm"><%= Year %>.<%=Month==0?"":MonthValue%></div>
								<ul>
								<% 
								for( MatchInfoDTO_1 mdto1 : matchList) {
									boolean is_home = (mdto1.getIs_home()).equals("True");
									System.out.println("is_home : " + is_home);
								%>
									<li class= "<%=is_home?"home":"away"%>">
										<div class="info1">
											<p><%=mdto1.getRound()%></p>
											<p><%=mdto1.getCompetition() %></p>
										</div>
										<div class="info2">
											<p>
												<i><%=mdto1.getM_date() %></i>
											...
											<p>
												<i class=<%=is_home?"home":"away" %>></i> <span><%=mdto1.getStadium()%></span>
											</p>
										</div>
										<div class="info3">
											<div class="left">
												<strong><%=is_home? mdto1.getHome_name() : mdto1.getAway_name()%></strong> <img
													src=<%=is_home? mdto1.getHome_logo() : mdto1.getAway_logo()%>>
											</div>
											<div class="center">
												<i class = <%=is_home?(((String)mdto1.getHome_winner()).equals("승")?"is_win":""):(((String)mdto1.getHome_winner()).equals("패")?"is_win":"")%>>
												<%=is_home? mdto1.getHome_fulltime(): mdto1.getAway_fulltime()%>
												</i> : <i class = <%=is_home?(((String)mdto1.getHome_winner()).equals("패")?"is_win":""):(((String)mdto1.getHome_winner()).equals("승")?"is_win":"")%>>
												<%=is_home? mdto1.getAway_fulltime(): mdto1.getHome_fulltime()%></i>
											</div>
											<div class="right">
												<img
													src=<%=is_home? mdto1.getAway_logo() : mdto1.getHome_logo()%>>
												<strong><%=is_home? mdto1.getAway_name() : mdto1.getHome_name()%></strong>
											</div>
										</div>
										<div class="info4 <%= mdto1.getHome_fulltime()!=""? "" : "btn" %>">
											<a class="hl2 <%= mdto1.getHome_fulltime()!=""? "" : is_home ? "homeM" : "awayM"%>"
												href=<%= mdto1.getHome_fulltime()!=""?"FcController?command=matchInfoResult&m_date="+ mdto1.getM_date() :is_home ? "FcController?command=ticketing":"#"%>>경기결과</a>
										</div>
									</li>
								<%		}
								%>
