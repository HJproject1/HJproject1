...
public class MatchInfoAction2 implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			mDTO1arr = MID2.MatchByDate2(informationDate,Month,Year);
		} catch (Exception e) {
		...
		request.setAttribute("MonthValue", MonthValue);
		request.setAttribute("showMatchInfo", mDTO1arr);
		request.getRequestDispatcher("MatchInfoV2_2.jsp").forward(request,  response);
	}
