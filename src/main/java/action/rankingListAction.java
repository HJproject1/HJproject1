...
import dto.RankDto;
import recordRankAPI.RecordRankDAO;
import recordRankAPI.recordRankDTO;
...
public class rankingListAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RecordRankDAO rhdao = new RecordRankDAO();
		ArrayList<recordRankDTO> rarr = new ArrayList<recordRankDTO>();
		try {
			rarr = rhdao.rankingList(Year);
		} catch (Exception e) {
		...
		request.setAttribute("rankingList", rarr);
		request.getRequestDispatcher("rankingList.jsp").forward(request,response);
	}
}
