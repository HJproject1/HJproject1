...
import dto.RankDto;
import recordRankAPI.RankingHelpDAO;
import recordRankAPI.rankingHelpDTO;

public class rankingHelpAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		RankingHelpDAO rhdao = new RankingHelpDAO();
		ArrayList<rankingHelpDTO> rarr = new ArrayList<rankingHelpDTO>();
		try {
			rarr = rhdao.helpList(Year);
		} catch (Exception e) {
		...
		request.setAttribute("helpList", rarr);
		request.getRequestDispatcher("rankingHelpList.jsp").forward(request,response);
	}
